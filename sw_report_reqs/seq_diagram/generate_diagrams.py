"""
Sequence Diagram Generator — SmartCare HMS (SRS v2.1)
Generates one PNG per use case using matplotlib.

Aligned with SRS v2.1 and generate_activity_diagrams.py (v2.1):
  • UC-01  Actor corrected to "Patient" (registration always creates Patient, FR-01).
           INSERT target corrected to patient profile only.
  • UC-03  Added Path A — change password while logged in (FR-03, matches activity diagram).
           Path B (forgot-password email link) retained; both paths now present.
  • UC-11  Added Notification Svc actor + patient notification step after invoice
           generation (FR-16 / FR-18, matches activity diagram uc11()).
  • UC-12  Added Notification Svc actor + payment confirmation notification step (FR-18).
  • Minor label polish throughout to match activity-diagram wording.
...
# FR-01: self-registration always creates a Patient account.
# Admin creates Doctor / Admin accounts via UC-04.
diagrams.append(dict(
    ...
    steps=[
        ...
        dict(type='msg', frm=0, to=1, label="Submit name, email, password, role=Patient, phone (opt.)"),
        ...
        dict(type='msg', frm=2, to=3, label="INSERT INTO patient profile"),
        dict(type='ret', frm=3, to=2, label="Patient profile created"),
...
# FR-03: Two paths per SRS v2.1 and activity diagram:
#   Path A — change password while logged in (current password required)
#   Path B — forgot-password via email reset link
diagrams.append(dict(
    title="Reset Password", uc_id="UC-03",
    actors=["User", "Browser", "API Server", "Database", "Email Service"],
    steps=[
        dict(type='divider', label="Path A — Change password (logged-in user)"),
        dict(type='msg', frm=0, to=1, label="Navigate to Change Password settings"),
        dict(type='ret', frm=1, to=0, label="Change-password form"),
        dict(type='msg', frm=0, to=1, label="Enter current password & new password"),
        dict(type='msg', frm=1, to=2, label="POST /api/auth/change-password  {currentPwd, newPwd}"),
        dict(type='msg', frm=2, to=3, label="SELECT user WHERE userId = ? (from JWT)"),
        dict(type='ret', frm=3, to=2, label="User record"),
        dict(type='self', frm=2, to=2, label="bcrypt.compare(currentPwd, hash)"),
        dict(type='self', frm=2, to=2, label="Hash new password (bcrypt)"),
        dict(type='msg', frm=2, to=3, label="UPDATE app_user SET password = newHash"),
        dict(type='ret', frm=3, to=2, label="Updated"),
        dict(type='ret', frm=2, to=1, label="200 OK — success"),
        dict(type='ret', frm=1, to=0, label="Success message shown"),
        dict(type='divider', label="Path B — Forgot password (email reset link)"),
        ...
        dict(type='msg', frm=2, to=3, label="Validate token (not expired); UPDATE password (bcrypt)"),
        dict(type='ret', frm=3, to=2, label="Updated"),
...
# FR-16: Invoice auto-generated on appointment COMPLETED.
# FR-18: Patient notified of new invoice (matches activity diagram uc11()).
diagrams.append(dict(
    title="Generate Invoice", uc_id="UC-11",
    actors=["System", "API Server", "Database", "Notification Svc"],
    steps=[
        ...
        dict(type='msg', frm=1, to=3, label="Notify patient: new invoice generated (FR-18)"),
        dict(type='ret', frm=3, to=1, label="Notification sent"),
...
# FR-17: Patient pays pending invoice via card or digital wallet.
# FR-18: Payment confirmation notification sent to patient.
diagrams.append(dict(
    title="Process Payment", uc_id="UC-12",
    actors=["Patient", "Browser", "API Server", "Payment Gateway", "Database", "Notification Svc"],
    steps=[
        ...
        dict(type='msg', frm=2, to=5, label="Notify patient: payment confirmed (FR-18)"),
        dict(type='ret', frm=5, to=2, label="Sent"),
